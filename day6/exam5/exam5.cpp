...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		CreateWindow(L"button", L"OK", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
			130, 0, 100, 25, hWnd, (HMENU)4001, hInst, NULL);
	}
		break;
    case WM_COMMAND:
        {
			...
			{
				HDC hdc = GetDC(hWnd);
				TCHAR szNum[256],szBuf[256];
				GetWindowText(GetDlgItem(hWnd, 3001), szNum, 256);
				int nNum = _wtoi(szNum);
				int nIndex=0;
				for (nIndex = 0; nIndex < 10; nIndex++) {
					swprintf_s(szBuf,L"%d * %d = %d",nNum,nIndex,nNum * nIndex);
					TextOut(hdc, 10, 50 + 15 * nIndex, szBuf, wcslen(szBuf));
				}
			}
			break;
