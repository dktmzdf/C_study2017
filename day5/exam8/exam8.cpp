...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		//CreateWindow(L"edit", NULL, WS_CHILD | WS_VISIBLE | CBS_AUTOHSCROLL | WS_BORDER,
		//	120, 30, 100, 25, hWnd, (HMENU)3002, hInst, NULL);
		CreateWindow(L"button", L"OK", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
			225, 30, 100, 25, hWnd, (HMENU)4001, hInst, NULL);
	}
	break;
    case WM_COMMAND:
        {
			...
			case 4001:
			{
				...
				TCHAR szNum[20];
				TCHAR szBuf[256];
				GetWindowText(GetDlgItem(hWnd, 3001), szNum, 20);
				num = _wtoi(szNum);

				if (num >10 && num <20) {
					swprintf_s(szBuf, 256, L"%d는 범위안에 있습니다.", num);
					TextOut(hdc, 0, 120, szBuf, wcslen(szBuf));
					
				}
				else{
					swprintf_s(szBuf, 256, L"%d는 범위안에 없습니다.", num);
					TextOut(hdc, 0, 120,szBuf, wcslen(szBuf));
				}
